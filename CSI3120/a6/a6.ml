...
let rec sublist_starting_at_index (n:int) (l:'a list) : 'a list = 
  match l with
  | [] -> raise NoSuchSublist
  | (h::t) -> if (n=1) then l else (sublist_starting_at_index (n-1) t)
...
let find_starting_at (p:'a->bool) (n:int) (l:'a list) : int = 
  if n < 1 then -2 else
    let l' = try (sublist_starting_at_index n l) with | NoSuchSublist -> [-1] in
    try (n + (find p 1 l')) with | NotFound -> 0 | Found f -> f;;

(* my testing predicate / statements 
let pred (a: 'a) : bool = a > 3;;

let _ = find_starting_at pred 1 [1;2;10;2];;
*)

(* PROBLEM 2 *)
(* Problem 2(a). Write a new version of "find" above called "find2",
   which takes two additional arguments, 
    one for a normal continuation
    and one for an errror continuation 
        (see the example on page 24 of the course notes for Chapter 8 of the Mitchell textbook). 
    Instead of raising exceptions, call the appropriate continuation.  *)

let rec find2 (p:'a->bool) (n:int) (l:'a list) (normal_cont: int->int) (error_cont: int) : int =
  match l with
  | [] -> error_cont
...
let find_and_continue (p:'a->bool) (n:int) (l:'a list) : int =
  let normal_cont (x: int) : int = x*2 in 
  let error_cont : int = Printf.printf "NotFound!\n"; 0 in
...
let rec exp2expression (e:exp) : expression = 
  match e with
  | Number e -> new number_exp e
  | Sum (e1, e2) -> new sum_exp (exp2expression e1) (exp2expression e2)


(* Test: *)
(exp2expression e_exp)#value;;
